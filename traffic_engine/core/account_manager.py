...
class AccountManager:
    ...
    async def get_available_account(
        ...
        Алгоритм:
        1. Фильтруем по сегменту (если указан)
        2. Фильтруем аккаунты без cooldown
        3. Фильтруем по дневному лимиту
        4. Сортируем по времени последнего использования
        5. Добавляем рандом

        Args:
            action_type: Тип действия (comment, invite, story_view, story_react)
            exclude_ids: ID аккаунтов для исключения
            segment: Сегмент (zozh/mama/business/student) — фильтр по аккаунтам
        ...
        async with get_session() as session:
            ...
            # Фильтрация по сегменту: аккаунт должен совпадать по сегменту
            # или не иметь сегмента (универсальный)
            if segment:
                query = query.where(
                    (UserBotAccount.segment == segment) | (UserBotAccount.segment.is_(None))
                )
