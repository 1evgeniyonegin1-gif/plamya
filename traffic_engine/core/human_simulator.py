...
class HumanSimulator:
    ...
    def get_sleep_until_morning_seconds(self) -> int:
        """
        Сколько секунд спать до начала рабочего дня + случайный сдвиг.

        Returns:
            Количество секунд до work_start + random(0-30min).
            0 если сейчас рабочие часы.
        """
        ...
        now = datetime.now()
        # Следующее утро
        if now.hour >= self.work_end:
            # После конца рабочего дня — спим до завтра
            tomorrow = now + timedelta(days=1)
            wake_time = tomorrow.replace(hour=self.work_start, minute=0, second=0, microsecond=0)
        else:
            # До начала рабочего дня — спим до сегодняшнего утра
            wake_time = now.replace(hour=self.work_start, minute=0, second=0, microsecond=0)

        sleep_seconds = int((wake_time - now).total_seconds())
        # Добавляем случайный сдвиг 0-30 минут (не просыпаться всем аккаунтам одновременно)
        jitter = random.randint(0, 1800)
        total = max(sleep_seconds + jitter, 60)

        logger.info(
            f"😴 Night sleep: {total // 3600}h {(total % 3600) // 60}m "
            f"(wake at ~{wake_time.strftime('%H:%M')} + {jitter // 60}min jitter)"
        )
        return total
