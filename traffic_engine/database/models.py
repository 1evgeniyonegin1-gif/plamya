...
class UserBotAccount(Base, TimestampMixin):
    ...
    # Сегмент контент-завода (привязка к нише)
    segment: Mapped[Optional[str]] = mapped_column(
        String(20), nullable=True, index=True
    )  # zozh, mama, business, student
    ...
    # Привязка к тематическому каналу (воронка уровень 2)
    linked_channel_id: Mapped[Optional[int]] = mapped_column(BigInteger, nullable=True)
    linked_channel_username: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
...
class TargetChannel(Base, TimestampMixin):
    ...
    # Сегмент канала (для маршрутизации по аккаунтам)
    segment: Mapped[Optional[str]] = mapped_column(
        String(20), nullable=True
    )  # zozh, mama, business, student, universal
    ...
    joined_at: Mapped[Optional[datetime]] = mapped_column(
        ...
        doc="Когда аккаунт вступил в канал — не комментить 24-48ч после"
    )
    has_antibot: Mapped[bool] = mapped_column(
        Boolean, default=False,
        doc="Есть ли anti-spam бот (Combot/Rose) — снижаем частоту"
    )
...
class TrafficAction(Base, TimestampMixin):
    ...
    # AI метаданные
    ai_model: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)  # claude, yandexgpt, deepseek
    ...
    # Smart comments метаданные
    comment_message_id: Mapped[Optional[int]] = mapped_column(
        BigInteger, nullable=True,
        doc="ID нашего коммента (для проверки ответов)"
    )
    strategy_used: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True,
        doc="Стратегия: smart/supportive/funny/expert"
    )
    relevance_score: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True,
        doc="Релевантность поста (0.0-1.0) из analyze_post()"
    )
    post_topic: Mapped[Optional[str]] = mapped_column(
        String(200), nullable=True,
        doc="Тема поста из AI-анализа"
    )

    # Reply tracking (обновляется Reply Checker)
    got_reply: Mapped[bool] = mapped_column(Boolean, default=False)
...
class StrategyEffectiveness(Base, TimestampMixin):
    """
    Эффективность стратегий комментирования (MAB Thompson Sampling).

    Хранит статистику по (segment, channel, strategy) для самообучения.
    Reply = 1.0 reward, Reaction = 0.5 reward.
    """
    __tablename__ = "traffic_strategy_effectiveness"

    id: Mapped[int] = mapped_column(primary_key=True)

    segment: Mapped[str] = mapped_column(String(20))
    channel_username: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    comment_strategy: Mapped[str] = mapped_column(String(50))
    time_slot: Mapped[Optional[str]] = mapped_column(String(20), nullable=True)  # morning, afternoon, evening
    post_topic: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)

    attempts: Mapped[int] = mapped_column(Integer, default=0)
    successes: Mapped[float] = mapped_column(Float, default=0)
    score: Mapped[float] = mapped_column(Float, default=0.5)

    # Indexes
    __table_args__ = (
        Index("idx_strategy_eff_lookup", "segment", "channel_username", "comment_strategy"),
    )

    def __repr__(self) -> str:
        return f"<StrategyEffectiveness {self.segment}/{self.comment_strategy}: {self.score:.0%}>"


class ChannelPost(Base, TimestampMixin):
    """
    Посты и сторис для тематических каналов.

    Используется для:
    - Автопостинга контента в каналы бот-аккаунтов
    - Публикации сторис от аккаунтов
    - Очереди и расписания постов
    """
    __tablename__ = "traffic_channel_posts"
    ...
    # Контент
    post_type: Mapped[str] = mapped_column(String(30))  # lifestyle, tips, motivation, product_mention, channel_promo
    ...
    # Story-специфичные поля
    is_story: Mapped[bool] = mapped_column(Boolean, default=False)
    story_privacy: Mapped[str] = mapped_column(String(20), default="public")  # public, contacts, close_friends
    ...
    # Indexes
    __table_args__ = (
        Index("idx_channel_post_status_scheduled", "status", "scheduled_at"),
    )
