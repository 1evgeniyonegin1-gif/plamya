...
class CommentPoster:
    ...
    async def post_comment(
        ...
        Args:
            ...
            relevance_score: Оценка релевантности поста (0-1)
            post_topic: Тема поста из анализа
            segment: Сегмент канала (zozh/mama/business/student) для выбора аккаунта

        Returns:
            True если успешно
        """
        # Получаем доступный аккаунт (с фильтрацией по сегменту)
        account = await self.account_manager.get_available_account("comment", segment=segment)
        if not account:
        ...
        # Проверяем similarity (не слишком ли похож на последние комменты)
        is_unique = await self._check_similarity(account.id, comment_text)
        if not is_unique:
            logger.info(f"⏭️ Comment too similar to recent ones, regeneration needed")
            return False
        ...
        try:
            ...
            if discussion_entity:
                ...
                # Сохраняем ID отправленного сообщения для трекинга ответов
                if sent_msg:
                    comment_message_id = sent_msg.id
                logger.info(f"Comment sent to discussion group of @{channel_username} (msg_id={comment_message_id})")
            else:
            ...
            # Записываем успешное действие с метаданными
            await self._log_action(
                account_id=account.id,
                tenant_id=self.account_manager.tenant_id,
                action_type="comment",
            ...
            # Уведомление об успешном комменте (тестовый режим)
            if self.notifier:
                await self.notifier.notify_action_success(
                    action_type="comment",
                    account_phone=account.phone,
                    segment=getattr(account, "segment", ""),
            ...
            if self.notifier:
                await self.notifier.notify_action_failed(
                    ...
                    error_message=f"FloodWait {e.seconds}s ({e.seconds / 3600:.1f}h)",
                )
            ...
            error_msg = f"{error_type} - {'joined discussion, retry next time' if joined else 'failed to join'}"
            if self.notifier:
                await self.notifier.notify_action_failed(
                    action_type="comment",
                    account_phone=account.phone,
            ...
            if self.notifier:
                await self.notifier.notify_action_failed(
                    ...
                    error_message=f"Account banned in @{channel_username}",
                )
            ...
            if self.notifier:
                await self.notifier.notify_action_failed(
                    ...
                    account_phone=account.phone,
                    segment=getattr(account, "segment", ""),
                    error_type="AuthKeyDuplicated",
                    error_message="Session used from different IP - session conflict!",
                )
    ...
    async def _log_action(
        ...
        """Записать действие в БД с метаданными анализа."""
    ...
    async def _check_similarity(self, account_id: int, comment_text: str, limit: int = 10) -> bool:
        """
        Проверяет, не слишком ли похож новый коммент на последние.

        Простая проверка на основе общих слов (Jaccard similarity).
        Если >60% совпадение с любым из последних комментов — блокируем.

        Returns:
            True если коммент достаточно уникален
        """
        try:
            async with get_session() as session:
                result = await session.execute(
                    select(TrafficAction.content).where(
                        TrafficAction.account_id == account_id,
                        TrafficAction.action_type == "comment",
                        TrafficAction.status == "success",
                        TrafficAction.content.isnot(None),
                    ).order_by(TrafficAction.created_at.desc()).limit(limit)
                )
            ...
            for old_comment in recent:
                ...
                if similarity > 0.6:
                    logger.warning(
                        f"⚠️ Similarity check: {similarity:.0%} match with recent comment. Blocking."
                    )
                    return False
