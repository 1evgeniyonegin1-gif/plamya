...
class StoryMonitor:
    ...
    - Просмотр stories + реакции через StoryReactor
    ...
    def __init__(
        ...
        self.story_reactor = StoryReactor()
    ...
    async def _monitoring_loop(self) -> None:
        ...
        while self._running:
            try:
                # 1. Проверка рабочих часов — ночью спим до утра
                ...
                # 5. Просмотреть story + реакция (30% шанс)
                try:
                    result = await self.story_reactor.view_and_react(
                        client=client,
                    ...
                    if result["viewed"]:
                        ...
                        if result["reacted"]:
                            logger.info(
                                f"🔥 Account {account.phone} reacted to story of user {target_user.user_id}"
                            )
                            await self.account_manager.mark_action(account.id, "story_reaction")
                ...
                if self.human_sim.should_take_break(self._session_actions):
                    ...
                    logger.info(f"☕ Story break: {break_min:.0f} min ({self._session_actions} views in session)")
