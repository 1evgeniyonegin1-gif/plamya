        ...
        while self._running:
            try:
                    ...
                    if result["invited"]:
                        ...
                        if self.notifier:
                            await self.notifier.notify_action_success(
                                ...
                                channel=chat.title or str(chat.chat_id),
                            )
                        ...
                        logger.debug(f"⏭️ Failed to invite user {target_user.user_id}: {error}")
                        if self.notifier and error != "unknown":
                ...
                if self.human_sim.should_take_break(self._session_actions):
                    ...
                    logger.info(f"☕ Invite break: {break_min:.0f} min ({self._session_actions} invites in session)")
