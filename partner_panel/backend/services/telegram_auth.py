...
class WebAppInitData:
    """Parsed initData from Telegram WebApp"""
    user: TelegramUser
    auth_date: datetime
    hash: str
...
class TelegramAuthService:
    """
    Service for validating Telegram Mini App authentication
    """
    ...
    def validate_init_data(self, init_data: str) -> Optional[WebAppInitData]:
        """
        Validate initData string from Telegram WebApp

        Args:
            init_data: The initData string from window.Telegram.WebApp.initData

        Returns:
            WebAppInitData if valid, None if invalid
        """
        try:
            ...
            # Parse auth_date (Telegram sends UTC timestamp)
    ...
    def is_data_fresh(self, init_data: WebAppInitData, max_age_seconds: int = 86400) -> bool:
        """
        Check if the auth data is fresh (not older than max_age_seconds)

        Args:
            init_data: Parsed WebAppInitData
